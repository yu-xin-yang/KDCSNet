...
class CheckPoint:
    def __init__(self, args):
        ...
        if not args.load:
            if not args.save:
                args.save = args.RFAN + '-' + str(args.sub_rate) + '-' + now
    ...
    def save(self, trainer, epoch, is_best=False):
        ...
        trainer.RFAN.save(self.dir, epoch, is_best=is_best)
        trainer.loss.save(self.dir)
        torch.save(trainer.Teacher_optimizer.state_dict(), os.path.join(self.dir, 'optimizer.pt'))
...
def normalize_255(img):
    image_res = img.cpu().detach().numpy().astype(np.float32)
...
def make_optimizer(args, my_model):
    ...
    elif args.Teacher_optimizer == 'ADAM':
...
def pad_image(img, imagesize):
    # print("img.shape: ",img.shape)
    [batch, channel, row, col] = img.shape
    row_pad = 0
    col_pad = 0
    if np.mod(row, imagesize) > 0:
        row_pad = imagesize - np.mod(row, imagesize)  # 求余数操作
    if np.mod(col, imagesize) > 0:
        col_pad = imagesize - np.mod(col, imagesize)  # 求余数操作，用于判断需要补零的数量

    pad_row = torch.zeros(batch, channel, row_pad, col)
    # print("pad:",pad_row.shape)
    Ipad = torch.cat([img, pad_row], dim=2)

    pad_col = torch.zeros(batch, channel, row + row_pad, col_pad)
    Ipad = torch.cat([Ipad, pad_col], dim=3)
    # print("Ipad.shape: ", Ipad.shape)

    # img1 = ToPILImage()(Ipad[0].data.cpu())
    # img1.show()

    return Ipad, row_pad, col_pad


def pad_delete(Ipad, row_pad, col_pad):
    [row, col] = Ipad.shape
    Ipad = Ipad[:row - row_pad, :col - col_pad]
    return Ipad
